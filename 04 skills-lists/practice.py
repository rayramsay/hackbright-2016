...
def n_long_words(words, n):
    ...
    n_long = [word for word in words if len(word) > n]
    return n_long
...
def halvesies(numbers):
    ...
    halved = [number/2.0 for number in numbers]
    return halved
...
def reverse_list_in_place(items):
    ...
    **Do not use** the python function `reversed()` or the method
    ...
    for index in range(0, len(items)/2):
        copy = items[index]
        items[index] = items[neg_counter]
        items[neg_counter] = copy
        neg_counter -= 1
